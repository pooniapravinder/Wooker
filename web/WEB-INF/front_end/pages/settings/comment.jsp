...
<%
    ...
    String comment_id = request.getParameter("kkt");
...
<html>
    ...
    <body><%@include file="../header.jsp"%>
        <%if(get_imgdetail!=null){
                ...
                byte do_comment = get_imgdetail.getDoComment();
                byte see_comment = get_imgdetail.getSeeComment();
                byte share = get_imgdetail.getShareOpt();
                byte permit_image = get_imgdetail.getCommentImage();
                byte permit_audio = get_imgdetail.getCommentAudio();
                byte permit_video = get_imgdetail.getCommentVideo();
                byte permit_file = get_imgdetail.getCommentFile();
                byte permit_sticker = get_imgdetail.getCommentSticker();
                String permit_extens = get_imgdetail.getCommentExtension();
                ...
                encrypt_val.encryptid((user_one+"_fdg34_65GuRiOPJd_jhj"+user_sqq).trim());
                String talking_id = encrypt_val.getEncryptedId();
        ...
        <p>Try out settings for comments</p>
        <form:form method="post" name="commentSetting" id="commentSetting" action="/user/comment/settings/">
            <label for="doComment">Who can comment on your account pic:</label><select id="doComment" name="doComment"><option value="opip"<%if(do_comment==1){out.print(" selected");}%>><spring:message code="editinfo.label.onlyme"/></option><option value="ibct"<%if(do_comment==2){out.print(" selected");}%>><spring:message code="editinfo.label.chatlist"/></option><option value="snzp"<%if(do_comment==3){out.print(" selected");}%>><spring:message code="editinfo.label.everyone"/></option><option value="szzi"<%if(do_comment==4){out.print(" selected");}%>><spring:message code="editinfo.label.myschool"/></option><option value="rqrq"<%if(do_comment==5){out.print(" selected");}%>><spring:message code="editinfo.label.mycollege"/></option><option value="zlpl"<%if(do_comment==6){out.print(" selected");}%>><spring:message code="editinfo.label.myuniversity"/></option><option value="iubn"<%if(do_comment==7){out.print(" selected");}%>><spring:message code="editinfo.label.mycity"/></option></select><br>
            <label for="picComment">Who can see who has commented on your account pic:</label><select id="picComment" name="picComment"><option value="dcwf"<%if(see_comment==1){out.print(" selected");}%>><spring:message code="editinfo.label.onlyme"/></option><option value="chii"<%if(see_comment==2){out.print(" selected");}%>><spring:message code="editinfo.label.chatlist"/></option><option value="nmmp"<%if(see_comment==3){out.print(" selected");}%>><spring:message code="editinfo.label.everyone"/></option><option value="cqkl"<%if(see_comment==4){out.print(" selected");}%>><spring:message code="editinfo.label.myschool"/></option><option value="xcwm"<%if(see_comment==5){out.print(" selected");}%>><spring:message code="editinfo.label.mycollege"/></option><option value="qrrz"<%if(see_comment==6){out.print(" selected");}%>><spring:message code="editinfo.label.myuniversity"/></option><option value="erry"<%if(see_comment==7){out.print(" selected");}%>><spring:message code="editinfo.label.mycity"/></option></select><br>
            Allow to media for comment:<input type="checkbox"<%if(permit_image==1){out.print(" checked");}%> name="typeImage" id="typeImage" value="true">Image&nbsp;<input type="checkbox"<%if(permit_audio==1){out.print(" checked");}%> name="typeAudio" id="typeAudio" value="true">Audio&nbsp;<input type="checkbox"<%if(permit_video==1){out.print(" checked");}%> name="typeVideo" id="typeVideo" value="true">Video&nbsp;<input type="checkbox"<%if(permit_file==1){out.print(" checked");}%> name="typeFile" id="typeFile" value="true">File&nbsp;<input type="checkbox"<%if(permit_sticker==1){out.print(" checked");}%> name="typeSticker" id="typeSticker" value="true">Sticker<br>
            ...
            <input type="hidden" name="next_eli" id="next_eli" value="<%if(next_eli!=null){out.print(next_eli);}%>" autocorrect="off" autocomplete="off" spellcheck="false"/>
            ...
            <input type="hidden" name="kkt" id="kkt" value="<%if(comment_id!=null){out.print(comment_id);}%>" autocorrect="off" autocomplete="off" spellcheck="false"/><br>
            ...
            <input type="hidden" name="refer" id="refer" value="<%if(refer!=null){out.print(refer);}%>" autocorrect="off" autocomplete="off" spellcheck="false"/>
