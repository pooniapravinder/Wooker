...
<html lang="en-US" id="wooker"><c:set var="requestURI" value="${requestScope['javax.servlet.forward.request_uri']}"/>
    ...
    <body>
        <%@include file="../header.jsp"%>
            ...
            <jsp:useBean id="getMessages" class="hibernate.query.chat.BlockedMessages"/>
            ...
            <c:choose>
                <c:when test="${(crypto.decrypt(param.pagination_backward))!=null}">
                    ...
                    <c:set var="messages" value="${getMessages.BlockedMessage(user,pagination_backward,false)}"/>
                </c:when>
                <c:when test="${(crypto.decrypt(param.pagination_forward))!=null}">
                    ...
                    <c:set var="messages" value="${getMessages.BlockedMessage(user,pagination_forward,true)}"/>
                </c:when>
                <c:otherwise>
                    <c:set var="messages" value="${getMessages.firstBlockedPageMessage(user)}"/>
                </c:otherwise>
            </c:choose>
                    ...
                    <c:forEach var="i" begin="${pagination_forward!=null ? 1 : (totalData==0 ? 1 : 0)}" end="${pagination_forward!=null ? totalData : (totalData>0 ? totalData-1 : totalData)}" step="1">
                        ...
                        <div class="__yttr">
                            <a href="/info/?sqq=${crypto.encodeURL(otherUser)}">
                                ...
                                <div class="hrier">
                                    <a href="/user/send/message/?sqq=${crypto.encodeURL(operId)}">${otherUserInfo.firstName} ${otherUserInfo.lastName}</a>
                                    <div class="ghj_hg">
