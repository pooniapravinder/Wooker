...
<html lang="en-US" id="wooker"><c:set var="requestURI" value="${requestScope['javax.servlet.forward.request_uri']}"/>
    ...
    <body>
        <%@include file="../header.jsp"%>
            ...
            <jsp:useBean id="getMessages" class="hibernate.query.chat.AlertsSentMessages"/>
            ...
            <c:choose>
                <c:when test="${(crypto.decrypt(param.pagination_backward))!=null}">
                    ...
                    <c:set var="messages" value="${getMessages.AlertSentMessage(user,pagination_backward,false)}"/>
                </c:when>
                <c:when test="${(crypto.decrypt(param.pagination_forward))!=null}">
                    ...
                    <c:set var="messages" value="${getMessages.AlertSentMessage(user,pagination_forward,true)}"/>
                </c:when>
                <c:otherwise>
                    <c:set var="messages" value="${getMessages.firstAlertSentPageMessage(user)}"/>
                </c:otherwise>
            </c:choose>
                                    ...
                                    <div class="qwri_dsa">
                                        ...
                                        <div class="hrier">
                                            ...
                                            <a href="/user/send/message/?sqq=${crypto.encodeURL(operId)}">${otherUserInfo.firstName} ${otherUserInfo.lastName}<c:if test="${countChatMsg>0}"> (${countChatMsg})</c:if></a>
                                                ...
                                                <div class="ghj_hg">
                                                    <a href="/user/action/cancel/?sqq=${crypto.encodeURL(operId)}<c:if test='${pagination_backward!=null}'>&amp;pagination_backward=${crypto.encodeURL(param.pagination_backward)}</c:if><c:if test='${pagination_forward!=null}'>&amp;pagination_forward=${crypto.encodeURL(param.pagination_forward)}</c:if>&amp;refer=rnJeLo-Iwk_YeJW">Cancel Request</a>
                                                </div>
                                            </div>
