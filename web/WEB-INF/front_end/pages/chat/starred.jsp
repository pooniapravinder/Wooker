...
<html lang="en-US" id="wooker"><c:set var="requestURI" value="${requestScope['javax.servlet.forward.request_uri']}"/>
    ...
    <body>
            ...
            <jsp:useBean id="getMessages" class="hibernate.query.chat.Starred"/>
            <c:choose>
                <c:when test="${(crypto.decrypt(param.pagination_backward))!=null}">
                    ...
                    <c:set var="messages" value="${getMessages.StarredMessage(user,pagination_backward,false)}"/>
                </c:when>
                <c:when test="${(crypto.decrypt(param.pagination_forward))!=null}">
                    ...
                    <c:set var="messages" value="${getMessages.StarredMessage(user,pagination_forward,true)}"/>
                </c:when>
                <c:otherwise>
                    <c:set var="messages" value="${getMessages.firstStarredPageMessage(user)}"/>
                </c:otherwise>
            </c:choose>
                    ...
                    <c:forEach var="i" begin="${pagination_forward!=null ? 1 : (totalData==0 ? 1 : 0)}" end="${pagination_forward!=null ? totalData : (totalData>0 ? totalData-1 : totalData)}" step="1">
                        ...
                        <c:forEach items="${data}" var="datas">
                            <c:choose>
                                <c:when test="${datas.getClass().name=='hibernate.mapping.Starredmessages'}">
                                    <c:if test="${i==(pagination_forward!=null ? 1 : 0)}"><c:set var="firstMsgId" value="${datas.starmsgId}"/></c:if>
                                    <c:if test="${i==(pagination_forward!=null ? totalData : totalData-1)}"><c:set var="lastMsgId" value="${datas.starmsgId}"/></c:if>
                                </c:when>
                        ...
                        <div class="__yttr">
                            <a href="/info/?sqq=${crypto.encodeURL(otherUser)}">
                                ...
                                <div class="hrier">
                                    <a href="/chat/option/perform/starred/?sqq=${crypto.encodeURL(operId)}">${otherUserInfo.firstName} ${otherUserInfo.lastName}</a>
                                    <div class="ghj_hg">
