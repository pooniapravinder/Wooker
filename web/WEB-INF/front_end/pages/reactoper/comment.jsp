...
<html lang="en-US" id="wooker">
    <head><c:set var="requestURI" value="${requestScope['javax.servlet.forward.request_uri']}"/><c:choose><c:when test="${requestURI.contains('accpic')}"><c:set var="requestPage" value="accpic"/></c:when><c:when test="${requestURI.contains('post')}"><c:set var="requestPage" value="post"/></c:when></c:choose>
                <meta name="keywords" content="Comment Panel at Wooker">
                <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
                <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0, user-scalable=no">
                <meta name="theme-color" content="#663399">
                <link rel="shortcut icon" href="//www.google.com/favicon.ico"/>
                <meta name="description" content="Comment Panel at Wooker">
            ...
            <div class="info_page main clearfix" id="web_view">
                ...
                <div class="ghjjk_fh">
                    ...
                    <div class="ghgff_gg">
                        ...
                        <div class="yul_sad">
                            ...
                            <a href="/comment/setings/">Settings</a></div></div>
                </div>
                ...
                <c:if test="${accpicoper.userId==user}">
                <%@include file="portions/showdata/selfcomment.jsp"%>
                </c:if>
                <c:if test="${accpicoper.userId!=user}">
                <%@include file="portions/showdata/notselfcomment.jsp"%>
                </c:if>
                <c:if test="${accpicoper.userId!=user}">
                <%@include file="portions/form/comment.jsp"%>
                </c:if>
            </div>
            </div>
        </div>
