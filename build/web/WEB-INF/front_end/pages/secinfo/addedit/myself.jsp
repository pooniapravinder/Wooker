...
<html lang="en-US" id="wooker">
    <head><c:set var="requestURI" value="${requestScope['javax.servlet.forward.request_uri']}"/>
        <meta name="keywords" content="Add your Myself">
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0, user-scalable=no">
        <meta name="theme-color" content="#663399">
        <link rel="shortcut icon" href="//www.google.com/favicon.ico"/>
        <meta name="description" content="Add your Myself">
        <meta name="google" content="notranslate">
        <title><c:choose><c:when test="${requestURI.contains('edit')}">Edit</c:when><c:otherwise>Add</c:otherwise></c:choose> Myself</title>
    ...
    <body>
        <div class="slide_layout" id="slide_layout"><div class="loading_text"><c:choose><c:when test="${requestURI.contains('edit')}">Updating</c:when><c:otherwise>Saving</c:otherwise></c:choose> myself....</div>
            <div class="loading">
        ...
        <div class="wrapper animate_in" id="wrapper">
            ...
            <div class="content">
                <h1><c:choose><c:when test="${requestURI.contains('edit')}">Edit</c:when><c:otherwise>Add</c:otherwise></c:choose> Myself</h1>
                ...
                <form:form method="post" name="addmyself" class="cfd__ds" modelAttribute="addmyself" action="">

                    <div class="form-element-fgrc">
                        <form:input path="myself" id="myself" class="info_input_box" autocorrect="off" autocomplete="off" spellcheck="false" required="true"/>
                        <form:label path="myself"><spring:message code="editmoreinfo.label.aboutme"/></form:label>
                            <span class="focus-border"></span>
                        </div>
                    <form:errors path="myself" class="add-errors"/>

                        <div class="form-element-ghtrr">
                        <form:label path="myselpri">Privacy:</form:label>
                        </div>
                        <div class="form-element-erta">
                            <div class="custom-select"><form:select path="myselpri" id="myselpri" class="custom-select-box"><c:forEach var="i" begin="1" end="6"><form:option value="${i}"><spring:message code="userprivacy.label.0${i}"/></form:option></c:forEach></form:select></div>
                                </div>

                                <div class="form-element-fgrc">
                        <form:input path="myseldesc" id="myseldesc" class="info_input_box" autocorrect="off" autocomplete="off" spellcheck="false"/>
                        <form:label path="myseldesc"><spring:message code="editmoreinfo.label.aboutdesc"/></form:label>
                            <span class="focus-border"></span>
                        </div>
                    <form:errors path="myseldesc" class="add-errors"/>
        ...
        <script>
            if (null !== h("addmyself"))
                h("addmyself").addEventListener("submit", function () {
                    as__bnm();
                });
