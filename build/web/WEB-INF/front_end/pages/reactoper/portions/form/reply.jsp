<div class="htul tty_da_das">
    <c:choose>
        <c:when test="${(user==userSqq.userTwo)&&(isDisable)}">
            <a href="/talking/user/oper/enable/?sqq=${crypto.encodeURL(param.sqq)}&amp;refer=weU_sJ-HQkjUsMl&amp;prerefer=${crypto.encodeURL(param.prerefer)}<c:if test='${page!=0}'>&amp;page=${page}</c:if>"><span class="yrr_jgf">Please enable talking to reply or click here</span></a>
        </c:when>
        <c:when test="${(user==userSqq.userTwo)&&(!isDisable)}">
            <a href="/talking/user/oper/disable/?sqq=${crypto.encodeURL(param.sqq)}&amp;refer=weU_sJ-HQkjUsMl&amp;prerefer=${crypto.encodeURL(param.prerefer)}<c:if test='${page!=0}'>&amp;page=${page}</c:if>"><span class="yrr_jgf">${replyUser}</span></a>
        </c:when>
        <c:otherwise>
            <span class="yrr_jgf">${replyUser}</span>
        </c:otherwise>
    </c:choose>
</div>
<div class="fdger_ghfdfd">
    ...
    <div class="tty_da_das">
        <form:form method="post" name="reply" modelAttribute="reply" action="${requestURI}?sqq=${crypto.encodeURL(param.sqq)}&refer=weU_sJ-HQkjUsMl&prerefer=${crypto.encodeURL(param.prerefer)}" enctype="multipart/form-data">

            <div class="dfhf_sd_hg">
                <form:textarea path="talking" id="hfgh_hfhf" class="frrt_zxfsr" placeholder="~${replyUser.replace(' ','').toLowerCase()}" autocorrect="off" autocomplete="off" spellcheck="false"/>
            ...
            <div class="gtrev_gfg yr_yu">
                ...
                <form:button name="reply" id="reply" class="form_talk_button" value="submit">Reply</form:button>
            </div>
        </form:form>
    </div>
</div>