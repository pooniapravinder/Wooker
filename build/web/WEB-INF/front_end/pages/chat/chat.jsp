...
<html lang="en-US" id="wooker"><c:set var="requestURI" value="${requestScope['javax.servlet.forward.request_uri']}"/>
    ...
    <body>
        <%@include file="../header.jsp"%>
            ...
            <jsp:useBean id="getMessages" class="hibernate.query.chat.ChatMessages"/>
            ...
            <c:choose>
                <c:when test="${(crypto.decrypt(param.pagination_backward))!=null}">
                    ...
                    <c:set var="messages" value="${getMessages.ChatMessage(user,pagination_backward,false)}"/>
                </c:when>
                <c:when test="${(crypto.decrypt(param.pagination_forward))!=null}">
                    ...
                    <c:set var="messages" value="${getMessages.ChatMessage(user,pagination_forward,true)}"/>
                </c:when>
                <c:otherwise>
                    <c:set var="messages" value="${getMessages.firstChatPageMessage(user)}"/>
                </c:otherwise>
            </c:choose>
                        ...
                        <c:forEach items="${data}" var="datas">
                            <c:choose>
                                <c:when test="${datas.getClass().name=='hibernate.mapping.Usermessages'}">
                                    ...
                                    <c:set var="onlineSec" value="${((now.time)-(getOnline.getOnline(actionPerformer)))/1000}"/>
