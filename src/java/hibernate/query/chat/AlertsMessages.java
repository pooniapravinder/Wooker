...
public class AlertsMessages {
    ...
    public List firstAlertPageMessage(long user) {
        Session sess = null;
        Transaction tx = null;
        List listdata = null;
        try {
    ...
    public List firstAlertSentPageMessage(long user) {
        Session sess = null;
        Transaction tx = null;
        List listdata = null;
        try {
            ...
            tx = sess.getTransaction();
            tx.begin();
            Query safehqlquery = sess.createQuery("from Usermessages tb1 inner join tb1.messages tb2 where tb1.autoId=(select max(tb3.autoId) from Usermessages tb3,Userchatlist tb4 where tb3.operId=tb1.operId and tb4.autoId=tb3.operId and tb4.request=true and tb4.userOne=:user and tb3.deletedOne<>:user and tb3.deletedTwo<>:user and tb3.getback=false) and tb1.autoId=tb2.msgId order by tb1.autoId desc");
            safehqlquery.setParameter("user", user);
            safehqlquery.setMaxResults(10);
            listdata = (List) safehqlquery.list();
            tx.commit();
    ...
    public List AlertMessage(long user, long pagination_id, boolean isForward) {
        Session sess = null;
        Transaction tx = null;
        List listdata = null;
        try {
